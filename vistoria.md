# Contexto legal atual da vistoria veicular no Brasil

🔹 Não existe hoje no Brasil uma vistoria veicular periódica obrigatória nacional baseada apenas na idade do veículo.

🔹 O que existe em vigor (Tier 0):

# Vistoria obrigatória em situações específicas previstas no Código de Trânsito Brasileiro (CTB):

* Transferência de propriedade
* Mudança de município/estado
* Veículos recuperados de roubo/furto
* Alterações estruturais

A vistoria atual serve para identificação e regularidade, não inspeção técnica completa.

# 🔹 Competência atual:

A vistoria é executada pelos Detrans estaduais, dentro de regras nacionais do Contran.

** O presidente da República não cria vistoria por decreto — isso exige lei aprovada pelo Congresso. **

🔹 Inspeção técnica veicular periódica:

** Já foi prevista no CTB no passado, mas nunca foi implementada nacionalmente. **

Tentativas anteriores fracassaram por custo, impacto social e resistência política.

#🔹 Situação do projeto citado:

O PL que propõe vistoria periódica para veículos com mais de 5 anos:

❌ Não é lei

❌ Não está em vigor

❌ Não cria imposto

✔️ Está em tramitação, sujeito a mudanças, rejeição ou arquivamento.

# ⚠️ Ponto-chave contra a desinformação

Confundir projeto em debate com lei em vigor e chamar vistoria de “imposto” é erro factual e enquadramento enganoso.
